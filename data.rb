...
def write_people(persons)
  # Check weither the person created is a student or a teacher
  temp = []
  ...
  # When created, the person is saved into the json file
  File.write('./data/people_file.json', JSON.generate(temp)).to_s
end

def read_people
  people = []
  people_from_json = JSON.parse(File.read('./data/people_file.json'))
